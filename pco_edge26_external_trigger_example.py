...
## ***Currently not succeeded in running this code -> see README.md***

'''Test the camera's ability to follow external triggers'''
ao = ni_PCIe_6738.DAQ(num_channels=1, rate=1e6, verbose=False)

camera = pco_edge26.Camera(verbose=True)
frames = 10
camera.apply_settings(frames, 100, 'max', 'max', 'binary+ASCII')
...
for i in range(frames):
    ...
    volt_period[jitter_px:jitter_px + exposure_px, 0] = 5
