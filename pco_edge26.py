...
class Camera:
    '''
    Basic device adaptor for PCO.edge 26 USB sCMOS camera. Many more
    ...
    def __init__(self,
        ...
        assert camera_name.value == b'pco.edge 26'
    ...
    def _get_pixel_rate(self):
        ...
        dwPixelRate = C.c_uint32(777) # 777 not valid -> should change
        dll.get_pixel_rate(self.handle, dwPixelRate)
        self.pixel_rate = dwPixelRate.value
        assert self.pixel_rate != 777
        if self.very_verbose:
    ...
    def _set_exposure_time_us(self, exposure_us, delay_us=0):
        ...
        assert 28 <= exposure_us <= 60e6, 'exposure %i out of range'%exposure_us
    ...
    def _get_roi(self):
        ...
        self.read_time_us = self.full_read_time_us * (self.height_px / 5120)
    ...
    def apply_settings(
        ...
        exposure_us=None,   # 28 <= type(int) <= 60,000,000
...
def legalize_image_size(
    height_px='max', width_px='max', name='PCO.edge26', verbose=True):
    """returns a nearby legal image size at the *center* of the camera chip"""
    height_step, width_step = 8, 32
    min_height, min_width, max_height, max_width = 16, 64, 5120, 5120
...
##    camera._reboot()
    ...
    # max fps test:
    frames = 1000
    camera.apply_settings(frames, 28, 'min', 'max', 'binary+ASCII', 'auto')
    images = np.zeros(
    ...
    print("\nMax fps = %0.2f\n"%(total_frames/time_s)) # ~ 6 -> 900 typical
    ...
    for i in range(num_acquisitions):
        ...
        expected_time_ms = 1e-3 * (camera.read_time_us + camera.exposure_us)
